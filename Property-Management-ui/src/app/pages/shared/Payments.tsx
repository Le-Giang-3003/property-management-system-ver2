...
export default function Payments({ role = "Landlord" }) {
  ...
  const filtered =
    statusFilter === "All"
      ? myPayments
      : myPayments.filter((p) => p.status === statusFilter);

  const summary = {
    totalPaid: myPayments
      .filter((p) => p.status === "Completed")
      .reduce((s, p) => s + p.amount, 0),
    totalPending: myPayments
      .filter((p) => p.status === "Pending")
      .reduce((s, p) => s + p.amount, 0),
    totalOverdue: myPayments
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-20">
        <div>
          ...
          <div className="page-desc">
            {role === "Tenant"
              ? "Theo dõi lịch sử và tình trạng thanh toán tiền thuê"
              : "Quản lý các khoản thanh toán từ người thuê"}
          </div>
        </div>
      </div>
      ...
      <div className="card">
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="fw-700 text-green">
        ...
        >
          Hiển thị {filtered.length} / {myPayments.length} thanh
          toán
        </div>
      </div>
        ...
        >
          ...
          >
            ...
            <div className="modal-body">
              ...
              {selectedPayment.lateFeeAmount && (
                <div className="info-row">
                  <span className="info-label">
                    Phí trễ hạn
                  </span>
                  <span className="info-value text-red">
                    {formatMoney(selectedPayment.lateFeeAmount)}
                  </span>
                </div>
              )}
